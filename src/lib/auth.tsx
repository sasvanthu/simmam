...
const MOCK_USERS: AdminUser[] = [
  ...
  },
  {
    ...
    email: 'reg@simmam.com',
    role: 'reg_team',
  },
  {
    id: '3',
    name: 'Developer Admin',
    email: 'admin@simmam.com',
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const hasPermission = (resource: string, action: string): boolean => {
    ...
    const permissions = user.role === 'coordinator' && resource === 'events' && user.assignedEvent
      ? [{ resource: 'events', actions: ['read'] }] // Coordinator can only see their assigned event
